...
function normalizeVehicleTypeCounts(rows = []) {
    ...
    (rows || []).forEach((row) => {
        ...
        if (['carro', 'auto', 'coche', 'vehiculo'].includes(tipo)) {
            map.carro += count;
            return;
        }

        if (['moto', 'motocicleta'].includes(tipo)) {
            map.moto += count;
            return;
        }

        if (['bici', 'bicicleta', 'bike'].includes(tipo)) {
            map.bici += count;
        }
    });
...
function updateDashboardStats(data) {
    // Actualizar contadores por tipo usando el total registrado en la empresa
    const map = normalizeVehicleTypeCounts(data.currentVehiclesByType || []);

    const carrosEl = document.getElementById('currCarros');
    const motosEl = document.getElementById('currMotos');
    const bicisEl = document.getElementById('currBicis');
    if (carrosEl) carrosEl.textContent = map.carro || 0;
    if (motosEl) motosEl.textContent = map.moto || 0;
    if (bicisEl) bicisEl.textContent = map.bici || 0;

    const incomeEl = document.getElementById('todayIncome');
    if (incomeEl) incomeEl.textContent = formatCurrency(data.todayIncome || 0);
